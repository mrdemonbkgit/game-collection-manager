...
interface UseHeroOptionsResult {
...
export function useHeroOptions(gameId: number | undefined): UseHeroOptionsResult {
  ...
  const fetchPage = useCallback(async (offset: number, append: boolean) => {
    ...
    try {
      const response = await fetchHeroOptions(gameId, 6, offset);
      ...
      if (!abortControllerRef.current?.signal.aborted) {
        setError(err instanceof Error ? err.message : 'Failed to fetch hero options');
      }
    } finally {
      if (!abortControllerRef.current?.signal.aborted) {
        setLoading(false);
